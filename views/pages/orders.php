                ...
                <?php if (!empty($orders)): ?>
                    <?php foreach ($orders as $order): ?>
                        ...
                        <tr>
                                ...
                                <?php if (!empty($license_display)): ?>
                                    <div class="compact-value">
                                        <span class="truncated"
                                            title="<?php echo htmlspecialchars($license_display); ?>"><?php echo compactStr($license_display, 8, 4); ?></span>
                                        <button class="btn btn-sm copy-btn"
                                            data-copy="<?php echo htmlspecialchars($license_display); ?>">Copy</button>
                            ...
                            <?php if ($isAdmin): ?>
                                <td>
                                    <?php if ($key && !empty($key['hwid'])): ?>
                                        <div class="compact-value">
                                            <span class="truncated"
                                                title="<?php echo htmlspecialchars($key['hwid']); ?>"><?php echo compactStr($key['hwid'], 8, 4); ?></span>
                                            <button class="btn btn-sm copy-btn"
                                                data-copy="<?php echo htmlspecialchars($key['hwid']); ?>">Copy</button>
                                            <button class="btn btn-sm toggle-btn"
                            ...
                            <?php endif; ?>
                            <td><?php if ($key) {
                                echo !empty($key['expires_at']) ? convertToVNTime($key['expires_at']) : 'Vĩnh viễn';
                            } else {
                                echo '-';
                            } ?>
                            </td>
                            <td><?php echo convertToVNTime($order['created_at']); ?></td>
                            <td>
                                    ...
                                    if ($isAdmin) {
                                        ...
                                        if (!empty($license_display)): ?>
                                            <?php if (empty($key['hwid'])): ?>
                                                <button class="btn"
                                                    onclick="openHWIDModal('<?php echo htmlspecialchars($license_display); ?>', '')">Nhập
                                                    HWID</button>
                                            <?php else: ?>
                                                <button class="btn"
                                                    onclick="openHWIDModal('<?php echo htmlspecialchars($license_display); ?>', '<?php echo htmlspecialchars($key['hwid'] ?? ''); ?>')">Đổi
                                                    HWID</button>
                                            <?php endif; ?>
                                        <?php endif; ?>
                                    <?php
                                        ...
                                        <?php if ($showLicenseDownload): ?>
                                            <a class="btn"
                                                href="/ShopToolNro/api/download/license_download.php?license=<?php echo urlencode($license_display); ?>">Tải
                                                sản phẩm</a>
                                        <?php elseif ($showProductLink): ?>
                                            <a class="btn" href="<?php echo htmlspecialchars($product['software_link']); ?>"
                                                target="_blank">Tải sản phẩm</a>
                                        <?php endif; ?>
    ...
    <div class="modal-content">
        ...
        <form id="hwidForm">
            <div class="modal-body">
                ...
                <p class="small mb-8">Bạn chỉ cần nhập <strong>HWID mới</strong>; nếu bạn là chủ sở hữu, hệ thống sẽ cập
                    nhật ngay.</p>
                <div class="form-group"><label>HWID mới</label><input type="text" id="hwid_new" name="new_hwid" required
                        class="w-100" placeholder="Nhập HWID mới (8-64 ký tự)" /></div>
            </div>
